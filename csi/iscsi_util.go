...
func (util *ISCSIUtil) DetachDisk(c iscsiDiskUnmounter, targetPath string) error {
	...
	cnt--
	if cnt != 0 && !done {
		klog.Warningf("Warning: remaining unmounts: %d", cnt)
		return nil
	}
...
func (util *ISCSIUtil) ListMounts() ([]string, error) {
	mounts := make([]string, 0)
	args := []string{"--noheadings", "--scsi", "--output", "MOUNTPOINT"}
	...
	if err != nil {
		...
		cmdError = fmt.Errorf("lsblk error: %s (%s)", formattedOutput, err.Error())
